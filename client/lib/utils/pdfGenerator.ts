...
export const generateCertificatePDF = async (application: Application): Promise<void> => {
  try {
    ...
    pdf.text('Office of Student Affairs', pageWidth / 2, 40, { align: 'center' });
    ...
    bodyText.forEach((line, index) => {
      ...
      } else if (line.startsWith('Roll Number:') || line.startsWith('Department:') || line.startsWith('Course:')) {
        pdf.setFontSize(12);
        pdf.setFont('helvetica', 'bold');
    ...
    // Download the PDF
    pdf.save(`no_due_certificate_${application.rollNumber}_${application.id}.pdf`);
...
export const generateSampleFormsPDF = (): void => {
  try {
    ...
    pdf.setFont('helvetica', 'normal');
    pdf.setTextColor(108, 117, 125);
    pdf.text('Sample Form & Instructions', pageWidth / 2, 30, { align: 'center' });
...
export const generateGuidelinesPDF = (): void => {
  try {
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setTextColor(13, 110, 253);
    pdf.text('NO DUE SYSTEM GUIDELINES', pageWidth / 2, 20, { align: 'center' });

    pdf.setFontSize(12);
    pdf.setFont('helvetica', 'normal');
    pdf.setTextColor(108, 117, 125);
    pdf.text('Complete Guide for Students', pageWidth / 2, 30, { align: 'center' });
    ...
    guidelines.forEach((line) => {
      ...
      if (line.endsWith(':') && !line.startsWith('•') && !line.includes('DEPARTMENT')) {
        pdf.setFont('helvetica', 'bold');
        pdf.setTextColor(13, 110, 253);
        pdf.setFontSize(11);
