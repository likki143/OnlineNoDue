...
export const createStudentNotificationEmail = (data: {
  ...
  const { studentName, studentEmail, applicationId, department, action, comments, officerName, dashboardUrl } = data;
...
export const sendCertificateReadyEmail = async (data: {
  ...
  try {
    ...
    if (emailSent) {
      ...
      return { success: false, error: 'Failed to send certificate ready email' };
    }
  } catch (error) {
